...
func Test_citypay_ApiKey(t *testing.T) {
	...
	t.Run("Test API Key generation with set nonce/date", func(t *testing.T) {

		dt, _ := time.Parse("2006-01-02T15:04:05", "2020-01-01T09:23:12")
		nonce, _ := hex.DecodeString("acb875aef083de292299bd69fcdeb5c5")

		key := openapiclient.GenerateApiKeyFor("PC2", "7G79TG62BAJTK669", nonce, dt)

		assert.Equal(t, key, "UEMyOkFDQjg3NUFFRjA4M0RFMjkyMjk5QkQ2OUZDREVCNUM1Ol6Q3tCMPsYvqNhFelRD2zQHYlZquBGCY/6ZpZ0AngTF")
	})

	t.Run("Test API Key generation with no nonce/date", func(t *testing.T) {
		key := openapiclient.GenerateApiKey("PC2", "7G79TG62BAJTK669")

		require.NotNil(t, key)
	})

	t.Run("Test API Key generations", func(t *testing.T) {
		data := [][]string{
		...
		for _, d := range data {
			...
			dt, _ := time.Parse("2006-01-02T15:04:05", d[3])
			nonce, _ := hex.DecodeString(d[2])

			key := openapiclient.GenerateApiKeyFor(d[0], d[1], nonce, dt)

			assert.Equal(t, key, d[4])

		}

		for _, d := range data {
			key := openapiclient.GenerateApiKey(d[0], d[1])
			require.NotNil(t, key)
		}
	})
}
