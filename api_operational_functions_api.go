...
type ApiRegisterTempKeyRequest struct {
	ctx             context.Context
	ApiService      *OperationalFunctionsApiService
	registerIpModel *RegisterIpModel
}

func (r ApiRegisterTempKeyRequest) RegisterIpModel(registerIpModel RegisterIpModel) ApiRegisterTempKeyRequest {
	r.registerIpModel = &registerIpModel
	return r
}

func (r ApiRegisterTempKeyRequest) Execute() (*Acknowledgement, *http.Response, error) {
	return r.ApiService.RegisterTempKeyExecute(r)
}

/*
RegisterTempKey Register Temp Key

Registers a temporary licence key.

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return ApiRegisterTempKeyRequest
*/
func (a *OperationalFunctionsApiService) RegisterTempKey(ctx context.Context) ApiRegisterTempKeyRequest {
	return ApiRegisterTempKeyRequest{
		ApiService: a,
		ctx:        ctx,
	}
...
//
//	@return Acknowledgement
func (a *OperationalFunctionsApiService) RegisterTempKeyExecute(r ApiRegisterTempKeyRequest) (*Acknowledgement, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodPost
		localVarPostBody    interface{}
		formFiles           []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "OperationalFunctionsApiService.RegisterTempKey")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v6/permissions/register-temp-ip"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.registerIpModel == nil {
		return localVarReturnValue, nil, reportError("registerIpModel is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.registerIpModel
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["cp-api-key"]; ok {
