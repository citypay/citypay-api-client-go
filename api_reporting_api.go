...
type ApiTransactionReportRequestRequest struct {
	ctx                      context.Context
	ApiService               *ReportingApiService
	transactionReportRequest *TransactionReportRequest
}

func (r ApiTransactionReportRequestRequest) TransactionReportRequest(transactionReportRequest TransactionReportRequest) ApiTransactionReportRequestRequest {
	r.transactionReportRequest = &transactionReportRequest
	return r
}

func (r ApiTransactionReportRequestRequest) Execute() (*BatchTransactionReportResponse, *http.Response, error) {
	return r.ApiService.TransactionReportRequestExecute(r)
}

/*
TransactionReportRequest Transaction Report Request
...
result state, merchant ID, and allows field selection or predefined response modes for optimal data handling.
	...
	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return ApiTransactionReportRequestRequest
*/
func (a *ReportingApiService) TransactionReportRequest(ctx context.Context) ApiTransactionReportRequestRequest {
	return ApiTransactionReportRequestRequest{
		ApiService: a,
		ctx:        ctx,
	}
...
//
//	@return BatchTransactionReportResponse
func (a *ReportingApiService) TransactionReportRequestExecute(r ApiTransactionReportRequestRequest) (*BatchTransactionReportResponse, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodPost
		localVarPostBody    interface{}
		formFiles           []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "ReportingApiService.TransactionReportRequest")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v6/transactions"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.transactionReportRequest == nil {
		return localVarReturnValue, nil, reportError("transactionReportRequest is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.transactionReportRequest
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["cp-api-key"]; ok {
